...
test('HTML, HTTP, malformed JSON, and network errors are not treated as data', async () => {
  await assert.rejects(
    lookupVehicle('PBC1234', {
      fetchImpl: async () => new Response('<html>challenge</html>', { headers: { 'content-type': 'text/html' } }),
    }),
    (error: unknown) => error instanceof GovernmentApiError && error.diagnostics?.status === 200,
  );
  await assert.rejects(
  ...
  await assert.rejects(
    lookupVehicle('PBC1234', { fetchImpl: async () => { throw new TypeError('offline'); } }),
    /No pudimos conectar/,
  );
});
...
test('timeout covers a stalled body and does not rely on fetch honoring abort', async () => {
  await assert.rejects(lookupVehicle('PBC1234', {
    timeoutMs: 5,
...
test('Fiscalía session and lookup receive separate timeout signals', async () => {
  ...
  await lookupFiscalia('PBC1234', {
    ...
    fetchImpl: async (url, init) => {
      ...
      return url === config.source.fiscaliaEntry ? new Response('session') : Response.json({});
    },
  });
